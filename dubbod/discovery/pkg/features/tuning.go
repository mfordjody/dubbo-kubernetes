...
var (
	...
	DebounceAfter = env.Register(
		...
		"The delay added to config/registry events for debouncing. This will delay the push by "+
			...
			" otherwise we'll keep delaying until things settle, up to a max of DUBBO_DEBOUNCE_MAX.",
	).Get()
	...
	EnableEDSDebounce = env.Register(
		...
		true,
		"If enabled, Dubbo will include EDS pushes in the push debouncing, configured by DUBBO_DEBOUNCE_AFTER and DUBBO_DEBOUNCE_MAX."+
	...
	XDSCacheIndexClearInterval = env.Register("DUBBO_XDS_CACHE_INDEX_CLEAR_INTERVAL", 5*time.Second,
