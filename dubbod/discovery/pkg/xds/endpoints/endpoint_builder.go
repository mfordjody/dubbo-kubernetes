...
func (b *EndpointBuilder) BuildClusterLoadAssignment(endpointIndex *model.EndpointIndex) *endpoint.ClusterLoadAssignment {
	...
	if !ok {
		...
		log.Infof("no shards found for service %s in namespace %s (cluster=%s, port=%d, svcPort.Name='%s', svcPort.Port=%d). "+
	...
	shards.RUnlock()
	log.Debugf("found shards for service %s (cluster=%s, port=%d, shardCount=%d, totalEndpointsInShards=%d)",
	...
	if len(allServicePortNames) > 0 {
		...
		log.Infof("service %s port %d (svcPort.Name='%s'), found endpoints with ServicePortNames: %v",
		...
		for _, ep := range eps {
			...
			if ep.ServicePortName != svcPort.Name {
				...
				if ep.ServicePortName == "" && len(b.service.Ports) == 1 {
					log.Debugf("including endpoint %s with empty ServicePortName (service has only one port '%s')",
						ep.FirstAddressOrNil(), svcPort.Name)
					// Continue processing this endpoint
				} else {
					portNameMismatchCount++
					filteredCount++
					log.Warnf("filtering out endpoint %s (port name mismatch: ep.ServicePortName='%s' != svcPort.Name='%s', EndpointPort=%d)",
			...
			if lbEp == nil {
				buildFailedCount++
				filteredCount++
				log.Debugf("buildLbEndpoint returned nil for endpoint %s", ep.FirstAddressOrNil())
				continue
			}
