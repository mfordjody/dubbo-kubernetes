		...
		if len(w.ResourceNames) == 0 && w.NonceSent != "" {
			...
			if w.TypeUrl == v3.ClusterType {
				...
				if ldsWatched != nil && ldsWatched.NonceSent != "" {
					...
					log.Debugf("CDS wildcard request, extracting cluster names from LDS")
					...
					if ldsGen != nil {
						...
						if len(ldsRes) > 0 {
							...
							if len(clusterNames) > 0 {
								...
								log.Debugf("extracted %d cluster names from LDS: %v", len(clusterNames), clusterNames)
							}
						}
					}
				}
					...
					if cdsGen != nil {
						...
						if len(cdsRes) > 0 {
							...
							if len(edsClusterNames) > 0 {
								...
								log.Debugf("extracted %d EDS cluster names from CDS: %v", len(edsClusterNames), edsClusterNames)
							}
						}
					}
				}
					...
					if ldsGen != nil {
						...
						if len(ldsRes) > 0 {
							...
							if len(routeNames) > 0 {
								...
								log.Debugf("extracted %d route names from LDS: %v", len(routeNames), routeNames)
							}
						}
					}
				}
		...
		if isDirectRequest {
			...
			if len(clusterNames) > 0 {
				...
				if cdsWatched == nil || cdsWatched.ResourceNames == nil || len(cdsWatched.ResourceNames) == 0 {
					...
					log.Debugf("LDS push completed, client already requested CDS, skipping auto-push")
				}
			}
			...
			if len(routeNames) > 0 {
				...
				if rdsWatched == nil || rdsWatched.ResourceNames == nil || len(rdsWatched.ResourceNames) == 0 {
					...
					if hasNewRoutes {
						...
						log.Debugf("LDS push completed, updating RDS watched resource with new routes: %v", routeNames)
						...
						log.Debugf("LDS push completed, RDS routes already watched: %v", routeNames)
					}
				}
			}
		}
		...
		if len(edsClusterNames) > 0 {
			...
			if edsWatched == nil {
				...
				if hasNewClusters {
					...
					log.Debugf("CDS push completed, updated EDS watched resource with new clusters: %v (waiting for client request)", edsClusterNames)
				} else {
					log.Debugf("CDS push completed, EDS clusters already watched: %v", edsClusterNames)
				}
			}
...
func extractRouteNamesFromLDS(listeners model.Resources) []string {
	...
	for _, r := range listeners {
		...
		if err := r.Resource.UnmarshalTo(ll); err != nil {
			log.Debugf("failed to unmarshal listener %s: %v", r.Name, err)
			continue
		}
					...
					if hcm.RouteSpecifier != nil {
						if rds, ok := hcm.RouteSpecifier.(*hcmv3.HttpConnectionManager_Rds); ok && rds.Rds != nil {
							...
							if routeName != "" {
								...
								log.Infof("found route name %s from FilterChain in listener %s", routeName, r.Name)
							}
						}
					}
				}
		...
		if ll.ApiListener != nil && ll.ApiListener.ApiListener != nil {
			...
			if hcm.RouteSpecifier != nil {
				if rds, ok := hcm.RouteSpecifier.(*hcmv3.HttpConnectionManager_Rds); ok && rds.Rds != nil {
					...
					if routeName != "" {
						routeNames.Insert(routeName)
						log.Debugf("found route name %s from ApiListener in listener %s", routeName, r.Name)
					}
				}
			}
		}
		...
		if strings.Contains(listenerName, ":") && !strings.HasPrefix(listenerName, "xds.dubbo.apache.org/grpc/lds/inbound/") {
			...
			if len(parts) == 2 {
				...
				if err == nil {
					...
					routeNames.Insert(routeName)
					log.Debugf("extracted route name %s from listener name %s", routeName, listenerName)
				}
			}
		}
	}
	log.Infof("extracted %d route names: %v", len(routeNames), routeNames.UnsortedList())
...
func extractEDSClusterNamesFromCDS(clusters model.Resources) []string {
	...
	for _, r := range clusters {
		...
		if err := r.Resource.UnmarshalTo(cl); err != nil {
			log.Debugf("failed to unmarshal cluster %s: %v", r.Name, err)
			continue
		}
