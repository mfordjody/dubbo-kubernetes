...
type colEntry struct {
	...
	StatusImport string
	// DubboClientImport represents the import alias for the API, taking into account Dubbo storing its API (spec)
	// separate from its client import
	// Example: apiclientnetworkingv1alpha3.
	DubboClientImport string
	// ClientGroupPath represents the group in the client. Example: NetworkingV1alpha3.
	ClientGroupPath string
	// ClientGetter returns the path to get the client from a kube.Client. Example: Dubbo.
	ClientGetter string
...
func buildInputs() (inputs, error) {
	...
	for _, r := range m.Resources {
		...
		e := colEntry{
			...
			DubboClientImport: toDubboImport(r.ProtoPackage, r.Version),
